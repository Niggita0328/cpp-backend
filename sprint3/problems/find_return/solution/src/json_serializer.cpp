...
json::value LostObjectToJson(const model::LostObject& lost_object) {
    json::object lost_obj;
    lost_obj["type"] = static_cast<std::uint64_t>(lost_object.type);
    lost_obj["pos"] = json::array{lost_object.position.x, lost_object.position.y};
    return lost_obj;
}
} // namespace json_serializer
