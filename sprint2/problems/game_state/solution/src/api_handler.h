...
void ApiHandler::HandleApiRequest(http::request<Body, http::basic_fields<Allocator>>&& req, Send&& send) {
    ...
    if (target == "/api/v1/game/players") {
        ...
        return send(this->MakeStringResponse(http::status::ok, json::serialize(players_obj), version, keep_alive, method));
    }
    ...
    if (target == "/api/v1/game/state") {
        ...
        return send(this->MakeStringResponse(http::status::ok, json::serialize(root_obj), version, keep_alive, method));
    }
