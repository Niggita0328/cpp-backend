...
LoadedGameData LoadGame(const std::filesystem::path& json_path) {
    ...
    if (const auto* retirement_time = root_obj.if_contains(keys::DOG_RETIREMENT_TIME)) {
        const double seconds = retirement_time->as_double();
        if (seconds < 0.0) {
            throw std::runtime_error("dogRetirementTime must be non-negative");
        }
        data.dog_retirement_timeout = std::chrono::duration_cast<std::chrono::milliseconds>(
