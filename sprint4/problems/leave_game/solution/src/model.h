...
struct LootGeneratorConfig {
    loot_gen::LootGenerator::TimeInterval period{0};
...
public:
    ...
    void SetBagCapacity(std::size_t capacity) {
        bag_capacity_ = capacity;
    }

    std::optional<std::size_t> GetBagCapacity() const noexcept {
...
class GameSession {
public:
    explicit GameSession(const Map* map_ptr, std::size_t bag_capacity, bool randomize_spawn = false);
    
    const Map* GetMap() const { return map_; }
    std::size_t GetBagCapacity() const noexcept { return bag_capacity_; }
    ...
    void SetLostObjects(std::vector<LostObject> objects, uint64_t next_id);
    uint64_t GetNextLostObjectId() const noexcept;
    void RemoveDog(Dog* dog);

private:
    void SpawnLostObjects(unsigned count);
...
public:
    ...
    void SetDefaultBagCapacity(std::size_t capacity) {
        default_bag_capacity_ = capacity;
    }

    std::size_t GetDefaultBagCapacity() const noexcept {
        return default_bag_capacity_;
    }

    void SetLootGeneratorConfig(LootGeneratorConfig config);
    const std::optional<LootGeneratorConfig>& GetLootGeneratorConfig() const noexcept;
    ...
    void Tick(std::chrono::milliseconds delta);
    void ClearSessions();
    const std::vector<std::unique_ptr<GameSession>>& GetSessions() const noexcept { return sessions_; }
    const std::unordered_map<Map::Id, size_t, util::TaggedHasher<Map::Id>>& GetSessionIdToIndex() const noexcept { return session_id_to_index_; }
...
private:
    ...
    std::size_t default_bag_capacity_ = 3;
    std::optional<LootGeneratorConfig> loot_generator_config_;
