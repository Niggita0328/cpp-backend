...
void GameSession::SetLostObjects(std::vector<LostObject> objects, uint64_t next_id) {
    uint64_t max_id = next_id;
    ...
    next_lost_object_id_ = max_id;
}

uint64_t GameSession::GetNextLostObjectId() const noexcept {
...
void GameSession::RemoveDog(Dog* dog) {
    ...
    auto it = std::remove(dogs_.begin(), dogs_.end(), dog);
    if (it != dogs_.end()) {
...
GameSession* Game::AddSession(const Map::Id& id) {
    ...
    auto session = std::make_unique<GameSession>(map, bag_capacity, randomize_spawn_points_);
    auto* session_ptr = session.get();
    sessions_.emplace_back(std::move(session));
    session_id_to_index_[id] = index;
    if (loot_generator_config_) {
        session_ptr->SetLootGeneratorConfig(*loot_generator_config_);
    }
    return session_ptr;
}
...
void Game::ClearSessions() {
    sessions_.clear();
    session_id_to_index_.clear();
}
